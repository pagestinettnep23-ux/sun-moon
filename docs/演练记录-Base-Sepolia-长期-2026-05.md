...
## Day 1 补充 - rc3 Stage 1 最终广播前人工批准表

### 1. 本次目标

准备 owner 在未来决定是否批准 Base Sepolia 测试网 Stage 1 前使用的人工批准表。当前所有批准项保持待填，仍不广播、不部署、不需要私钥。
...
必读文档确认
网络和钱包确认
Stage 1 允许的 12 笔测试网交易
Stage 1 明确不批准的内容
预测地址确认
owner 批准语句填写区，当前全部待填
操作员执行前必须再次检查的条件
Stage 1 成功后必须先复核
绝对停止条件
```
...
下一步只建议 owner 人工阅读批准表，并决定是否填写批准区。

## Day 1 补充 - rc3 Stage 1 测试网地址防混淆标注

### 1. 本次目标

根据 owner 提醒，把 Stage 1 批准表里的 3 个钱包地址明确标成 Base Sepolia 测试网地址，避免误认为主网正式地址。

### 2. 标注范围

```text
0x2F6E887c6058deE520f9468a1022E3480A6334D3 = Stage 1 测试网执行钱包，不是主网地址
0x6E22b2e6fFAA30Fe75B71d53d1eC469b4e97A986 = 测试网管理员钱包，不是主网地址
0x277ba3Cf597CdAaF958C301db3cF6a631F793039 = 测试网协议经费钱包，不是主网地址
```
...
当前没有使用私钥
当前没有使用真实资金
主网正式地址仍必须另看 MAINNET_* 那组公开参数
```

## Day 1 补充 - rc3 Stage 1 owner 宽泛批准回复记录

### 1. 本次目标

记录 owner 当前回复，并判断是否足以进入执行版。当前仍不广播、不部署、不需要私钥。
...
## Day 1 补充 - rc3 Stage 1 执行版命令草案（不广播）

### 1. 本次目标

根据 owner 明确指令“只准备 Stage 1 执行版命令草案，不广播”，新增一份命令草案文档。当前仍不广播、不部署、不需要私钥、不使用真实资金。
...
```text
当前 staged draft 脚本不是可广播脚本
当前脚本固定 broadcastAllowed=false
当前脚本固定 executionBlocked=true
当前脚本拒绝 EXECUTE_BASE_SEPOLIA_RC3_STAGE=1
当前脚本拒绝 PRIVATE_KEY 非空
...
未来真正执行版不能直接复用当前脚本
未来真正执行版必须满足的字段
未来真正执行版最多只能覆盖 12 笔 Stage 1 测试网交易
未来真正执行版禁止项
签名边界
owner 下一次人工确认模板
Stage 1 成功后的后复核要求
...
## Day 1 补充 - rc3 Stage 1 真正执行命令审阅版（不广播）

### 1. 目标

根据 owner 明确确认，准备 Base Sepolia 测试网 rc3 Stage 1 真正执行命令审阅版。

本步骤仍不广播、不部署、不需要私钥、不使用真实资金。
...
命令审阅版不是执行版
当前真正可运行执行命令仍未生成
审阅版命令外形保留 LOCAL_SIGNER_OPTION_PENDING_FINAL_REVIEW 待填占位
审阅版不包含私钥
审阅版不包含 --private-key
审阅版不包含 Stage 2/3、Base 主网或真实资金
```
