...
contract PrepareBaseSepoliaRc3Stage1RecoveryContinuationDraft is Script {
    ...
    bytes32 internal constant LABEL_PARTIAL_MOON_TOKEN = "PARTIAL_MOON_TOKEN";
    ...
    uint256 internal constant CONTINUATION_TRANSACTIONS_PLANNED = 9;
    uint256 internal constant DEFAULT_EXPECTED_CONTINUATION_NONCE = 19;
    ...
    address internal constant DEFAULT_PARTIAL_MOON_TOKEN =
        0x92dC3B8056cA62A7dbc5c1C339891B45463bEe71;
    ...
    struct ContinuationConfig {
        ...
        uint256 expectedContinuationNonce;
    ...
    error BaseSepoliaStage1ContinuationNotConfirmed(uint256 chainId);
    error ContinuationNonceMismatch(uint256 expected, uint256 actual);
    ...
    error Stage1ContinuationNotConfirmed();
    error UnsupportedChain(uint256 chainId);
    error UsdcDecimalsMismatch(uint8 expected, uint8 actual);

    function run() external returns (ContinuationPlan memory plan) {
        plan = prepare(_loadConfig());
    }

    function prepare(ContinuationConfig memory config)
        public
        returns (ContinuationPlan memory plan)
    {
        _validateConfig(config);

        plan = _buildPlan(config);
        _validateContinuationState(plan);

        if (config.executeRequested) {
            _executeContinuation(config, plan);
            _validateExecutedContinuation(config, plan);
        }

        _logPlan(plan);
    }

    function _loadConfig() private view returns (ContinuationConfig memory config) {
        config = ContinuationConfig({
            ...
            partialMoonToken: vm.envOr("PARTIAL_MOON_TOKEN", DEFAULT_PARTIAL_MOON_TOKEN),
            expectedContinuationNonce: vm.envOr(
                "BASE_SEPOLIA_RC3_STAGE1_CONTINUATION_EXPECTED_NONCE",
                DEFAULT_EXPECTED_CONTINUATION_NONCE
            ),
            moonLaunchDelay: vm.envOr("MOON_LAUNCH_DELAY", uint256(0)),
            continuationConfirmed: vm.envOr(
                    "CONFIRM_BASE_SEPOLIA_RC3_STAGE1_CONTINUATION_DRAFT", uint256(0)
                ) == 1,
            executeRequested: vm.envOr("EXECUTE_BASE_SEPOLIA_RC3_STAGE1_CONTINUATION", uint256(0))
                == 1,
            privateKeyPresent: bytes(vm.envOr("PRIVATE_KEY", string(""))).length != 0
        });
    }

    function _validateConfig(ContinuationConfig memory config) private view {
        ...
        if (block.chainid == BaseV4Addresses.BASE_SEPOLIA_CHAIN_ID) {
            if (!config.continuationConfirmed) {
                revert BaseSepoliaStage1ContinuationNotConfirmed(block.chainid);
            }
            if (config.usdcToken != BaseV4Addresses.BASE_SEPOLIA_USDC) {
                revert InvalidAddress(LABEL_USDC_TOKEN);
            }
        }
        if (config.executeRequested && !config.continuationConfirmed) {
            revert Stage1ContinuationNotConfirmed();
        }
        ...
        _requireAddress(config.partialMoonToken, LABEL_PARTIAL_MOON_TOKEN);
        ...
        _requireCode(config.partialMoonToken, LABEL_PARTIAL_MOON_TOKEN);
    }

    function _buildPlan(ContinuationConfig memory config)
        private
        view
        returns (ContinuationPlan memory plan)
    {
        plan.chainId = block.chainid;
        plan.continuationConfirmed = config.continuationConfirmed;
        plan.executeRequested = config.executeRequested;
        plan.privateKeyPresent = config.privateKeyPresent;
        plan.broadcastAllowed = config.executeRequested && config.continuationConfirmed;
        plan.executionBlocked = !plan.broadcastAllowed;
        plan.simulationOnly = !plan.broadcastAllowed;
        plan.remainingTransactionsPlanned = CONTINUATION_TRANSACTIONS_PLANNED;
        plan.stage1CoreDeployer = config.sepoliaDeployer;
        plan.stage1CoreDeployerNonce = vm.getNonce(config.sepoliaDeployer);
        plan.expectedContinuationNonce = config.expectedContinuationNonce;
        plan.continuationNonceMatches =
            plan.stage1CoreDeployerNonce == config.expectedContinuationNonce;
        ...
        plan.partialMoonToken = config.partialMoonToken;
        ...
        plan.partialMoonTokenOwner = MoonToken(config.partialMoonToken).owner();
        ...
        plan.remainingAddressCollision =
            plan.predictedMoonCurveHasCode || plan.predictedCreate2HookDeployerHasCode;
    }

    function _executeContinuation(ContinuationConfig memory config, ContinuationPlan memory plan)
        private
    {
        ...
        MoonToken moonToken = MoonToken(config.partialMoonToken);
    ...
    function _validateContinuationState(ContinuationPlan memory plan) private pure {
        if (!plan.continuationNonceMatches) {
            revert ContinuationNonceMismatch(
                plan.expectedContinuationNonce, plan.stage1CoreDeployerNonce
            );
        }
        ...
        if (plan.partialMoonTokenOwner != plan.stage1CoreDeployer) {
            revert ExistingStage1StateMismatch(
                LABEL_PARTIAL_MOON_TOKEN, plan.stage1CoreDeployer, plan.partialMoonTokenOwner
            );
        }
        ...
        if (plan.partialMoonTokenMinter != address(0)) {
            revert ExistingStage1StateMismatch(
                LABEL_PARTIAL_MOON_TOKEN, address(0), plan.partialMoonTokenMinter
            );
        }
    ...
    function _validateExecutedContinuation(
        ContinuationConfig memory config,
        ContinuationPlan memory plan
    ) private view {
    ...
    function _logPlan(ContinuationPlan memory plan) private pure {
        console2.log("Base Sepolia rc3 Stage 1 recovery continuation draft");
        console2.log("chainId:", plan.chainId);
        console2.log("continuationConfirmed:", plan.continuationConfirmed);
        ...
        console2.log("expectedContinuationNonce:", plan.expectedContinuationNonce);
        console2.log("continuationNonceMatches:", plan.continuationNonceMatches);
        ...
        console2.log("PARTIAL_MOON_TOKEN:", plan.partialMoonToken);
        ...
        console2.log(
            "Next step:", "manual review only unless owner separately approves continuation"
        );
    }
}
